...
function Map() {
  ...
  useEffect(() => {
    svg.current = d3.select('.map_container').append('svg').attr('width', width).attr('height', height);
  ...
  const changeLocation = useCallback(() => {
    ...
    if (coordinates[currentLocation]) {
      ...
      d3.transition().duration(1000).tween('tween', () => t => {
  ...
  }, [coordinates, currentLocation, p2, r2, path, projection, getAreaFill, getCircleFill, getCircleStroke]);

  const drawMap = useCallback(() => {
    d3.json(`${window.location.href.includes('unctad.org') ? 'https://storage.unctad.org/2022-ldc_report/' : './'}assets/data/WorldMap_Territories_54030.topojson`).then((topology) => {
