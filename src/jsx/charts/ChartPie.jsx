...
function PieChart({
  allow_decimals = true, data, export_title_margin = 0, idx, labels = true, note = false, source, standalone = false, subtitle = false, tick_interval = 1, title, xlabel = '', ymax = undefined, ymin = undefined, ystep = 1
}) {
      ...
      chart: {
        events: {
          ...
          load() {
            ...
            this.renderer.image(`${window.location.href.includes('unctad.org') ? 'https://storage.unctad.org/2022-ldc_report/' : './'}assets/img/unctad_logo.svg`, 5, 15, 80, 100).add();
          }
        },
        height: chartHeight,
        marginRight: 20,
  ...
  return (
    <div className="chart_container" style={(standalone) ? { minHeight: chartHeight, maxWidth: '600px', marginTop: '40px' } : { minHeight: chartHeight }}>
      <div ref={chartRef}>
